...
def open_data_t187ap23_L() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["股票代號"], metadata=row))
        docs.append(Document(page_content=row["公司名稱"], metadata=row))
        docs.append(Document(page_content=row["違規事由"], metadata=row))
...
def open_data_t187ap04_L() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["主旨 "], metadata=row))
        docs.append(Document(page_content=row["公司代號"], metadata=row))
        docs.append(Document(page_content=row["公司名稱"], metadata=row))
        docs.append(Document(page_content=row["事實發生日"], metadata=row))
        docs.append(Document(page_content=row["說明"], metadata=row))
    return docs
...
def announcement_BFZFZU_T() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["Number"], metadata=row))
        docs.append(Document(page_content=row["Code"], metadata=row))
        docs.append(Document(page_content=row["Name"], metadata=row))
        docs.append(Document(page_content=row["Date"], metadata=row))
    return docs
...
def open_data_t187ap02_L() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["出表日期"], metadata=row))
        docs.append(Document(page_content=row["公司代號"], metadata=row))
        docs.append(Document(page_content=row["公司名稱"], metadata=row))
        docs.append(Document(page_content=row["大股東名稱"], metadata=row))
...
def announcement_punish() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["Code"], metadata=row))
        docs.append(Document(page_content=row["Name"], metadata=row))
        docs.append(Document(page_content=row["Detail"], metadata=row))
        docs.append(Document(page_content=row["LinkInformation"], metadata=row))
    return docs
...
def open_data_t187ap17_L() -> pd.DataFrame:
    ...
    for row in data:
        docs.append(Document(page_content=row["公司代號"], metadata=row))
        docs.append(Document(page_content=row["公司名稱"], metadata=row))
        docs.append(Document(page_content=row["營業收入(百萬元)"], metadata=row))
        docs.append(
            Document(page_content=row["毛利率(%)(營業毛利)/(營業收入)"], metadata=row)
        )
        docs.append(
            Document(
                page_content=row["營業利益率(%)(營業利益)/(營業收入)"], metadata=row
            )
        )
        docs.append(
            Document(
                page_content=row["稅前純益率(%)(稅前純益)/(營業收入)"], metadata=row
            )
        )
        docs.append(
            Document(
                page_content=row["稅後純益率(%)(稅後純益)/(營業收入)"], metadata=row
            )
        )
...
t187ap04_L = StructuredTool.from_function(
    open_data_t187ap04_L, description=PROMPT_t187ap04_L, return_direct=False
)


t187ap23_L = StructuredTool.from_function(
    open_data_t187ap23_L, description=PROMPT_t187ap23_L, return_direct=False
)


t187ap02_L = StructuredTool.from_function(
    open_data_t187ap02_L, description=PROMPT_t187ap02_L, return_direct=False
)


t187ap17_L = StructuredTool.from_function(
    open_data_t187ap17_L, description=PROMPT_t187ap17_L, return_direct=False
)


BFZFZU_T = StructuredTool.from_function(
    announcement_BFZFZU_T, description=PROMPT_BFZFZU_T, return_direct=False
)


punish = StructuredTool.from_function(
    announcement_punish, description=PROMPT_punish, return_direct=False
)
